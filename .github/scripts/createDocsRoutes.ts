...
const platformNames = {
    ieattaClassic: 'ieatta-classic',
    newIeatta: 'new-ieatta',
} as const;
 ...
 * @param platformName - Ieatta Classic or New Ieatta
...
function run() {
    const ieattaClassicArticleHubs = fs.readdirSync(`${docsDir}/articles/${platformNames.ieattaClassic}`);
    const newIeattaArticleHubs = fs.readdirSync(`${docsDir}/articles/${platformNames.newIeatta}`);

    const ieattaClassicRoute = routes.platforms.find((platform) => platform.href === platformNames.ieattaClassic);
    const newIeattaRoute = routes.platforms.find((platform) => platform.href === platformNames.newIeatta);

    if (ieattaClassicArticleHubs.length !== ieattaClassicRoute?.hubs.length) {
        console.error(warnMessage(platformNames.ieattaClassic));
        process.exit(1);
    }
    ...
    createHubsWithArticles(ieattaClassicArticleHubs, platformNames.ieattaClassic, ieattaClassicRoute.hubs);
