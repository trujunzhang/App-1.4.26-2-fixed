...
class ReactNativeHybridApp(reactContext: ReactApplicationContext) :
  ...
  override fun shouldUseStaging(isStaging: Boolean) {
    Log.d(NAME, "`shouldUseStaging` should never be called in standalone `New Ieatta` app")
  }

  override fun closeReactNativeApp(shouldSignOut: Boolean, shouldSetNVP: Boolean) {
    Log.d(NAME, "`closeReactNativeApp` should never be called in standalone `New Ieatta` app")
  }

  override fun completeOnboarding(status: Boolean) {
    Log.d(NAME, "`completeOnboarding` should never be called in standalone `New Ieatta` app")
  }
  ...
  override fun switchAccount(
    ...
    Log.d(NAME, "`switchAccount` should never be called in standalone `New Ieatta` app")
  }

  override fun sendAuthToken(authToken: String?) {
    Log.d(NAME, "`sendAuthToken` should never be called in standalone `New Ieatta` app")
  }
}
