...
import PopoverReportActionContextMenu from './expPages/home/report/ContextMenu/PopoverReportActionContextMenu';
...
type IeattaProps = IeattaOnyxProps;
    ...
    useEffect(() => {
        setTimeout(() => {
            BootSplash.getVisibilityStatus().then((status) => {
                ...
                if (status === 'visible') {
                    const propsToLog: Omit<IeattaProps & {isAuthenticated: boolean}, 'children' | 'session'> = {
                        isCheckingPublicRoom,
                        updateRequired,
                        updateAvailable,
                        isSidebarLoaded,
...
export default withOnyx<IeattaProps, IeattaOnyxProps>({
    isCheckingPublicRoom: {
