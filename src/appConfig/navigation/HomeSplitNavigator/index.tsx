...
function HomeSplitNavigator() {
    ...
    return (
        <FocusTrapForScreens>
            <Split.Navigator
                persistentScreens={[SCREENS.APP_SIDE_BAR]}
                // Issue:
                //    Cannot read properties of undefined (reading 'pattern')
                //    So we need to add a 'SCREENS.APP_SIDE_BAR' screen
                //    Not to be 'SCREENS.HOME'
                sidebarScreen={SCREENS.APP_SIDE_BAR}
                defaultCentralScreen={SCREENS.RESTAURANT}
                parentRoute={route}
            ...
            >
                ...
                <Split.Screen
                    ...
                    initialParams={{restaurantId: restaurantIdInSidebar}}
                    getComponent={loadRestaurantScreen}
                />
                {Object.entries(CENTRAL_PANE_HOME_SCREENS).map(([screenName, componentGetter]) => {
                    return (
                        <Split.Screen
                            key={screenName}
                            name={screenName as keyof Screens}
                            getComponent={componentGetter as any}
                        />
                    );
                })}
            </Split.Navigator>
