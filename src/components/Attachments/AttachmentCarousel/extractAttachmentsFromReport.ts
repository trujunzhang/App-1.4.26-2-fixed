    ...
    const htmlParser = new HtmlParser({
        onopentag: (name, attribs) => {
            ...
            if (name === 'img' && attribs.src) {
                const ieattaSource = attribs[CONST.ATTACHMENT_SOURCE_ATTRIBUTE];
                const source = tryResolveUrlFromApiRoot(ieattaSource || attribs.src);
                if (uniqueSources.has(source)) {
                    return;
                }
                ...
                attachments.unshift({
                    ...
                    source,
                    isAuthTokenRequired: Boolean(ieattaSource),
                    file: {name: fileName},
                    isReceipt: false,
