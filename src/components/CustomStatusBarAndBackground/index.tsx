...
function CustomStatusBarAndBackground({isNested = false}: CustomStatusBarAndBackgroundProps) {
    ...
    // The prev and current status bar background color refs are initialized with the splash screen background color so the status bar color is changed from the splash screen color to the expected color atleast once on first render - https://github.com/Ieatta/App/issues/34154
    ...
    const updateStatusBarStyle = useCallback(
        (listenerID?: number) => {
            ...
            // https://github.com/Ieatta/App/issues/34154#issuecomment-1898519399
