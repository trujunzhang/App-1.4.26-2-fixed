...
type FBUserAvatarWithImagePickerProps = {
...
function FBUserAvatarWithImagePicker({
    DefaultAvatar = () => null,
    style,
    disabledStyle,
    editIconStyle,
...
}: FBUserAvatarWithImagePickerProps) {
    ...
    const isValidResolution = (image: FileObject): Promise<boolean> =>
        getImageResolution(image).then(({height, width}) => {
            return height >= CONST.FBAVATAR_MIN_HEIGHT_PX && width >= CONST.FBAVATAR_MIN_WIDTH_PX && height <= CONST.FBAVATAR_MAX_HEIGHT_PX && width <= CONST.FBAVATAR_MAX_WIDTH_PX;
        });
...
FBUserAvatarWithImagePicker.displayName = 'FBUserAvatarWithImagePicker';

export default FBUserAvatarWithImagePicker;
