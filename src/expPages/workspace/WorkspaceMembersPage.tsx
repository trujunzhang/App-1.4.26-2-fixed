...
function WorkspaceMembersPage({personalDetails, invitedEmailsToAccountIDsDraft, route, policy, session, currentUserPersonalDetails, isLoadingReportData = true}: WorkspaceMembersPageProps) {
    ...
    const getUsers = useCallback((): MemberOption[] => {
        ...
        Object.entries(policy?.employeeList ?? {}).forEach(([email, policyEmployee]) => {
            ...
            // If this policy is owned by Ieatta then show all support (ieatta.com or team.ieatta.com) emails
            // We don't want to show guides as policy members unless the user is a guide. Some customers get confused when they
            // see random people added to their policy, but guides having access to the policies help set them up.
            if (PolicyUtils.isIeattaTeam(details?.login ?? details?.displayName)) {
                if (policyOwner && currentUserLogin && !PolicyUtils.isIeattaTeam(policyOwner) && !PolicyUtils.isIeattaTeam(currentUserLogin)) {
                    return;
                }
            }
