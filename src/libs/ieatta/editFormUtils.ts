...
let restaurantDraft: OnyxTypes.Form | null = {};
Onyx.connect({
...
let eventDraft: OnyxTypes.Form | null = {};
Onyx.connect({
...
let recipeDraft: OnyxTypes.Form | null = {};
Onyx.connect({
...
let reviewDraft: OnyxTypes.Form | null = {};
Onyx.connect({
...
function navigationToEditRestaurant(restaurantId: string) {
    updateDraftValuesForEditModelId({
        draftID: ONYXKEYS.FORMS.IEATTA_RESTAURANT_DRAFT,
...
function navigationToEditEvent(eventId: string) {
    updateDraftValuesForEditModelId({
        draftID: ONYXKEYS.FORMS.IEATTA_EVENT_DRAFT,
        editFormUniqueId: eventId,
        lastEditFormUniqueId: lodashGet(eventDraft ?? {}, 'editFormUniqueId', ''),
    });
    Navigation.navigate(ROUTES.EDIT_EVENT.getRoute(eventId));
}
function navigationToEditRecipe(recipeId: string) {
    updateDraftValuesForEditModelId({
        draftID: ONYXKEYS.FORMS.IEATTA_RECIPE_DRAFT,
        editFormUniqueId: recipeId,
        lastEditFormUniqueId: lodashGet(recipeDraft ?? {}, 'editFormUniqueId', ''),
    });
    Navigation.navigate(ROUTES.EDIT_RECIPE.getRoute(recipeId));
}
...
function navigationToEditReview({reviewId = CONST.IEATTA_EDIT_MODEL_NEW, relatedId, reviewType}: {reviewId?: string; relatedId: string; reviewType: string}) {
    updateDraftValuesForEditModelId({
        draftID: ONYXKEYS.FORMS.IEATTA_REVIEW_DRAFT,
