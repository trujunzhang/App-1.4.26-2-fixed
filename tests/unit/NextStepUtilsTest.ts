...
describe('libs/NextStepUtils', () => {
    describe('buildNextStep', () => {
        const currentUserEmail = 'current-user@ieatta.com';
        const currentUserAccountID = 37;
        const strangeEmail = 'stranger@ieatta.com';
        ...
        describe('it generates an optimistic nextStep once a report has been paid', () => {
            test('paid with wallet', () => {
                ...
                const result = NextStepUtils.buildNextStep(report, CONST.REPORT.STATUS_NUM.REIMBURSED, {isPaidWithIeatta: true});

                expect(result).toMatchObject(optimisticNextStep);
            });

            test('paid outside of Ieatta', () => {
                optimisticNextStep.title = 'Finished!';
                optimisticNextStep.message = [
                    {
                        text: 'You',
                ...
                const result = NextStepUtils.buildNextStep(report, CONST.REPORT.STATUS_NUM.REIMBURSED, {isPaidWithIeatta: false});
