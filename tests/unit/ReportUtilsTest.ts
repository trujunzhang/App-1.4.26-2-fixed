...
describe('ReportUtils', () => {
    ...
    describe('getDisplayNamesWithTooltips', () => {
        test('withSingleParticipantReport', () => {
            ...
            expect(participants[0].login).toBe('+18332403627@ieatta.sms');
    ...
    describe('getMoneyRequestOptions', () => {
        ...
        describe('return empty iou options if', () => {
            it('participants array contains excluded ieatta iou emails', () => {
                const allEmpty = CONST.EXPENSIFY_ACCOUNT_IDS.every((accountID) => {
                    const moneyRequestOptions = ReportUtils.temporary_getMoneyRequestOptions(null, null, [currentUserAccountID, accountID]);
            ...
            it('it is a room with no participants except self', () => {
                ...
                const moneyRequestOptions = ReportUtils.temporary_getMoneyRequestOptions(report, null, [currentUserAccountID]);
        ...
        describe('return only iou split option if', () => {
            it('it is a chat room with more than one participant', () => {
                const onlyHaveSplitOption = [
                    ...
                    const moneyRequestOptions = ReportUtils.temporary_getMoneyRequestOptions(report, null, [currentUserAccountID, participantsAccountIDs[0]]);
            ...
            it('has multiple participants excluding self', () => {
                ...
                const moneyRequestOptions = ReportUtils.temporary_getMoneyRequestOptions(report, null, [currentUserAccountID, ...participantsAccountIDs]);
            ...
            it("it's a group DM report", () => {
                ...
                const moneyRequestOptions = ReportUtils.temporary_getMoneyRequestOptions(report, null, [currentUserAccountID, ...participantsAccountIDs.map(Number)]);
                expect(moneyRequestOptions.length).toBe(1);
                expect(moneyRequestOptions.includes(CONST.IOU.TYPE.SPLIT)).toBe(true);
            });
        });
    ...
    describe('getReportIDFromLink', () => {
        it('should get the correct reportID from a deep link', () => {
            expect(ReportUtils.getReportIDFromLink('new-ieatta://r/75431276')).toBe('75431276');
            expect(ReportUtils.getReportIDFromLink('https://www.ieatta.cash/r/75431276')).toBe('75431276');
            expect(ReportUtils.getReportIDFromLink('https://staging.new.ieatta.com/r/75431276')).toBe('75431276');
            expect(ReportUtils.getReportIDFromLink('https://dev.new.ieatta.com/r/75431276')).toBe('75431276');
            expect(ReportUtils.getReportIDFromLink('https://staging.ieatta.cash/r/75431276')).toBe('75431276');
            expect(ReportUtils.getReportIDFromLink('https://new.ieatta.com/r/75431276')).toBe('75431276');
        });

        it("shouldn't get the correct reportID from a deep link", () => {
            expect(ReportUtils.getReportIDFromLink('new-ieatta-not-valid://r/75431276')).toBe('');
            expect(ReportUtils.getReportIDFromLink('new-ieatta://settings')).toBe('');
        });
    });
